...
json_file = open('models/85/diff_final_model.json', 'r')
loaded_model_json = json_file.read()
json_file.close()
model = model_from_json(loaded_model_json)
model.load_weights('models/85/diff_final_model.h5')

# Define constants for audio recording
format = pyaudio.paInt16
channels = 1
filename = "test/hello/disgust.wav"
duration=5
sample_rate=44100
chunk_size=1024
duration=5
...
def preprocess_audio(filename, sample_rate=22050, n_mfcc=40, offset_s=0.5):
    ...
    pitches, magnitudes = librosa.piptrack(y=y_trimmed, sr=sample_rate)
    ...
    # Intensity
    intensity = np.mean(np.abs(y_trimmed))
    features.append(intensity)

    # MFCCs (Mel-frequency cepstral coefficients)
    mfcc = librosa.feature.mfcc(y=y_trimmed, sr=mfcc_sample_rate, n_mfcc=n_mfcc)
    ...
    # Chroma features
    chroma = librosa.feature.chroma_stft(y=y_trimmed, sr=sample_rate)
...
with open('meta/diff_scaler.pkl', 'rb') as f:
